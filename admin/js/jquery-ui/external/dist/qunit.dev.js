...
(function (window) {
  ...
  Test.prototype = {
    ...
    run: function run() {
      ...
      try {
        ...
        QUnit.ok(false, "Died on test #" + (this.assertions.length + 1) + ": " + e.message + " - " + QUnit.jsDump.parse(e)); // else next test will carry the responsibility
  ...
  (function () {
    ...
    config.filter = urlParams.filter; // Figure out if we're running the tests from a server or not
  ...
  addEvent(window, "load", function () {
    QUnit.begin({}); // Initialize the config, saving the execution queue
    ...
    var jsDump = {
      ...
      typeOf: function typeOf(obj) {
        ...
        if (obj === null) {
          ...
          type = "node";
        } else if (_typeof(obj) === "object" && typeof obj.length === "number" && obj.length >= 0) {
          type = "array";
        } else {
          type = _typeof(obj);
        }
      ...
      parsers: {
        ...
        node: function node(_node) {
          ...
          for (var a in QUnit.jsDump.DOMAttrs) {
            var val = _node[QUnit.jsDump.DOMAttrs[a]];
